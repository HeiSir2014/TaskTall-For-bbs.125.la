...
void CRenderEngine::DrawImage(HDC hDC, HBITMAP hBitmap, const RECT& rc, const RECT& rcPaint,
							  const RECT& rcBmpPart, const RECT& rcCorners, const RECT& rcInCorners, bool bAlpha, 
							  BYTE uFade, bool bHole, bool bTiledX, bool bTiledY)
{
	ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);


	bool bLeftScale = false;
	bLeftScale = (rcInCorners.left +rcInCorners.right+rcInCorners.top +rcInCorners.bottom != 0);
	...
	if( lpAlphaBlend && (bAlpha || uFade < 255) ) {
		...
		// left-top
		if(!bLeftScale && rcCorners.left > 0 && rcCorners.top > 0 ) {
			rcDest.left = rc.left;
			rcDest.top = rc.top;
		...
		// right-top
		if(!bLeftScale && rcCorners.right > 0 && rcCorners.top > 0 ) {
		...
		// left
		if( rcCorners.left > 0 || (bLeftScale && rcInCorners.left > 0)) {

			rcDest.left = rc.left;
			rcDest.top = bLeftScale?( rc.top+((rc.bottom-rc.top)- (rcBmpPart.bottom-rcBmpPart.top - rcInCorners.top - rcInCorners.bottom))/2 ) :( rc.top + rcCorners.top);
			rcDest.right = bLeftScale ? rcInCorners.left:rcCorners.left;
			rcDest.bottom =  bLeftScale?(rcBmpPart.bottom-rcBmpPart.top - rcInCorners.top - rcInCorners.bottom):(rc.bottom - rc.top - rcCorners.top - rcCorners.bottom);
			rcDest.right += rcDest.left;
			rcDest.bottom += rcDest.top;
			if( ::IntersectRect(&rcTemp, &rcPaint, &rcDest) ) {
				rcDest.right -= rcDest.left;
				rcDest.bottom -= rcDest.top;
				lpAlphaBlend(hDC, rcDest.left, rcDest.top, rcDest.right, rcDest.bottom, hCloneDC, \
					rcBmpPart.left, rcBmpPart.top +(bLeftScale ? rcInCorners.top:rcCorners.top), bLeftScale ? rcInCorners.left:rcCorners.left, bLeftScale ? (rcBmpPart.bottom-rcBmpPart.top - rcInCorners.top - rcInCorners.bottom):(rcBmpPart.bottom - \
					rcBmpPart.top - rcCorners.top - rcCorners.bottom), bf);
			}
		}
		// right
		if(!bLeftScale && rcCorners.right > 0 ) {
			rcDest.left = rc.right - rcCorners.right;
			rcDest.top = rc.top + rcCorners.top;
			rcDest.right = rcCorners.right;
			rcDest.bottom = rc.bottom - rc.top - rcCorners.top - rcCorners.bottom;
		...
		// left-bottom
		if(!bLeftScale && rcCorners.left > 0 && rcCorners.bottom > 0 ) {
			rcDest.left = rc.left;
		...
		// bottom
		if(!bLeftScale && rcCorners.bottom > 0 ) {
			rcDest.left = rc.left + rcCorners.left;
			rcDest.top = rc.bottom - rcCorners.bottom;
			rcDest.right = rc.right - rc.left - rcCorners.left - rcCorners.right;
			rcDest.bottom = rcCorners.bottom;
		...
		// right-bottom
		if(!bLeftScale && rcCorners.right > 0 && rcCorners.bottom > 0 ) {
			rcDest.left = rc.right - rcCorners.right;
			rcDest.top = rc.bottom - rcCorners.bottom;
			rcDest.right = rcCorners.right;
			rcDest.bottom = rcCorners.bottom;
					  ...
					  TDrawInfo& drawInfo)
{
	...
	if( drawInfo.pImageInfo == NULL ) {
		...
		while( *pstrImage != _T('\0') ) {
			...
			if( !sValue.IsEmpty() ) {
				...
				else if( sItem == _T("incorner") ) {
					drawInfo.rcInCorner.left = _tcstol(sValue.GetData(), &pstr, 10);  ASSERT(pstr);    
					drawInfo.rcInCorner.top = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr);    
					drawInfo.rcInCorner.right = _tcstol(pstr + 1, &pstr, 10);  ASSERT(pstr);    
					drawInfo.rcInCorner.bottom = _tcstol(pstr + 1, &pstr, 10); ASSERT(pstr);
				}
	...
	DrawImage(hDC, drawInfo.pImageInfo->hBitmap, rcDest, rcPaint, drawInfo.rcBmpPart, drawInfo.rcCorner, drawInfo.rcInCorner,
...
void CRenderEngine::DrawColor(HDC hDC, const RECT& rc, DWORD color)
{
    ...
    else
    {
        ...
        *pDest = color;
		...
		RECT rcCorners = {0};
		RECT rcInCorners = {0};
        DrawImage(hDC, hBitmap, rc, rc, rcBmpPart, rcCorners, rcInCorners, true, 255);
						...
						if( pImageInfo ) {
                            ...
                            if( pt.x + iWidth > rc.right && pt.x > rc.left && (uStyle & DT_SINGLELINE) == 0 ) {
                                ...
                                if( bDraw && bLineDraw ) {
									...
									CDuiRect rcInCorner(0, 0, 0, 0);
                                    DrawImage(hDC, pImageInfo->hBitmap, rcImage, rcImage, rcBmpPart, rcCorner,rcInCorner, \
