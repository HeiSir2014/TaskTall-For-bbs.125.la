...
void CListUI::ReSizeItemWidth()
{
	int nSize = GetCount();
	for (int i = 0; i < nSize; ++i)
	{
		CControlUI* pCtrl = GetItemAt(i);
		if (pCtrl && _tcsstr(pCtrl->GetClass(), _T("ListContainerElementUI")))
		{
			((CListContainerElementUI*)pCtrl)->Invalidate();
		}
		else
		{
			break;
...
bool CListUI::Add(CControlUI* pControl)
{
	...
	else if(m_pHeader && _tcsstr(pControl->GetClass(), _T("ListContainerElementUI")) != NULL ) {
		CListContainerElementUI *pListEle = static_cast<CListContainerElementUI*>(pControl);
		int nPos = 0;

		TListInfoUI* pInfo = GetListInfo();
		for( int i = 0; i < pInfo->nColumns && i < pListEle->GetCount(); i++ )
		{
			CControlUI* pCtrl = pListEle->GetItemAt(i);
			if ( pCtrl)
			{
				pCtrl->SetFixedWidth((int)(pInfo->rcColumn[i].right - pInfo->rcColumn[i].left));
			}
		}
	}
...
void CListUI::DoEvent(TEventUI& event)
{
	...
	case UIEVENT_KEYUP:
		switch( event.chKey ) {
		case VK_CONTROL:
			if (m_bMultiSelect & m_bCtrlStatus)
			{
				m_bCtrlStatus = false;
			}
			return;
...
{
	int i = -1;
	if( (i = m_nSelectedIndex.Find((LPVOID)iIndex)) > -1)
		m_nSelectedIndex.Remove(i);
...
bool CListUI::SelectItem(int iIndex, bool bTakeFocus)
{
    ...
    if(!m_bCtrlStatus) {
		...
		if (m_nSelectedIndex.GetSize() > 0 )
		{
			for (int i = m_nSelectedIndex.GetSize()-1 ; i >=0 ; --i)
			{
				CControlUI* pControl = GetItemAt((int)m_nSelectedIndex[i]);
				if( pControl != NULL) {
					IListItemUI* pListItem = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
	...
	if( m_nSelectedIndex.Find((LPVOID)iIndex) == -1)
		m_nSelectedIndex.Add((LPVOID)iIndex);
...
void CListUI::SetItemVLineColor(DWORD dwVLineColor)
{
	m_ListInfo.dwVLineColor = dwVLineColor;
	Invalidate();
}
...
void CListHeaderItemUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONUP )
    {
        if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
			...
			{
				((CListUI*)(GetParent()->GetParent()))->ReSizeItemWidth();
			}
        }
...
bool CListElementUI::Select(bool bSelect)
{
	...
	else if( !bSelect && m_pOwner != NULL ) m_pOwner->UnSelectItem(m_iIndex);
    Invalidate();
...
void CListContainerElementUI::Invalidate()
{
	...
	if (m_pOwner)
	{
		...
		TListInfoUI* pInfo = m_pOwner->GetListInfo();
		for( int i = 0; i < pInfo->nColumns && i < GetCount(); i++ )
		{
			CControlUI* pCtrl = this->GetItemAt(i);
			if ( pCtrl)
			{
				pCtrl->SetFixedWidth((int)(pInfo->rcColumn[i].right - pInfo->rcColumn[i].left));
				//nPos += headerItem->GetWidth() + headerItem->GetPadding().left + headerItem->GetPadding().right + headerItem->GetSepWidth();
			}
		}
	}
...
void CListContainerElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
{
	...
	if ( pInfo->dwLineColor != 0 ) {
		RECT rcLine = { m_rcItem.left, m_rcItem.bottom + m_rcPadding.bottom/2, m_rcItem.right, m_rcItem.bottom +m_rcPadding.bottom/2 };
		CRenderEngine::DrawLine(hDC, rcLine, 1, GetAdjustColor(pInfo->dwLineColor));
	}

	if ( pInfo->dwVLineColor != 0 ) {
		int width =0;
		for (int i = 0; i < pInfo->nColumns;++i)
		{
			width += pInfo->rcColumn[i].right - pInfo->rcColumn[i].left;
			RECT rcLine = { m_rcItem.left + width, m_rcItem.top, m_rcItem.left + width, m_rcItem.bottom };
